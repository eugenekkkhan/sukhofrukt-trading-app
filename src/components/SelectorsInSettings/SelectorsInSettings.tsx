...
const SelectorsInSettings = () => {
  ...
  const [initialWorkingSide, setInitialWorkingSide] =
    useState<WorkingSideType | null>(null);
  const [initialTimeFrame, setInitialTimeFrame] =
    useState<TimeFrameType | null>(null);
  ...
  useEffect(() => {
    if (id && !isFetched)
      fetchUserOptions(id).then((res: AxiosResponse<UserOptionsType>) => {
        //set working side
        setInitialWorkingSide(
          dictionary[res.data.workingSide] as WorkingSideType,
        );
      ...
      <div className="flex-row flex-wrap">
        <div style={{ width: "360px", zIndex: "20" }}>
          <SelectComponent
            ...
            value={workingSide}
            setValue={
              setWorkingSide as Dispatch<
                SetStateAction<string | undefined | null>
              >
            }
          />
        </div>
          ...
          <ButtonWithIcon
            ...
            onClick={() => {
              if (id && workingSide && initialTimeFrame) {
                const parsedWorkingSide: WorkingSideType = dictionary[
                  workingSide
                ] as WorkingSideType;
                ChangeOptionsOnServer(
                  parseInt(id),
                  parsedWorkingSide,
                  initialTimeFrame,
                );
          ...
          <ButtonWithIcon
            ...
            onClick={() => {
              if (id && initialWorkingSide && timeFrame) {
                const parsedInitialWorkingSide: WorkingSideType = dictionary[
                  initialWorkingSide
                ] as WorkingSideType;
                ChangeOptionsOnServer(
                  parseInt(id),
                  parsedInitialWorkingSide,
                  timeFrame,
                );
              }
            }}
